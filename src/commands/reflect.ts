...
export async function reflectCommand(name: string, opts: ReflectOptions): Promise<void> {
  ...
  if (opts.apply) {
    ...
    // Deduplicate: don't append if a "## Learned Rules" section already exists at end
    const separator = "\n\n---\n_Auto-generated by `ccmux reflect`_\n\n";
    const append = separator + reflection + "\n";
    await fs.writeFile(claudeMdPath, existing + append, "utf-8");
